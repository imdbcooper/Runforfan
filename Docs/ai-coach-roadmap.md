...
На 14 июля 2026 года этапы 1-4 завершены. Текущий код включает immutable action ledger, compensating rollback, event-driven recalculation assessments, historical resolver, deterministic Weekly Review, подтверждаемые недельные стратегии и Hybrid Conversational Coach.

Stage 4 реализован как русскоязычный explanation/clarification layer с typed bounded context, strict provider output, citation/safety validation, provider repair/fallback, explicit memory confirmation и отдельным preview handoff. Turn endpoint не создаёт preview и не применяет mutation; preview повторно авторизуется сервером, а apply остаётся в существующих deterministic services. Provider attempts сохраняют только metadata/fingerprints, feature flag по умолчанию выключен. Пройдены unit, PostgreSQL, runtime, adversarial и desktop/mobile gates. Production deployment этапов 3-4 не выполнялся.

Следующая точка продолжения: этап 5, Recovery и Wearable Signals. Новые автоматические сигналы должны иметь provenance/freshness, не переопределять subjective check-in и не расширять mutation authority LLM.

## Порядок ближайших инкрементов

1. Спроектировать normalized recovery signal model с provenance, freshness и quality.
2. Добавить wearable/self-report conflict handling с conservative fallback и уточнением.
3. Расширить Athlete State bounded recovery context без передачи raw vendor payloads.
4. Добавить regression fixtures для missing/stale/conflicting wearable signals.
5. Выпустить Stage 5 только после safety replay и ограниченного rollout gate.
