...
**Статус: в работе. Инкременты 6.1, 6.2 и 6.3a завершены; полный этап 6 не завершён.**
...
- Stage 6.3b остаётся незавершённым: до staffed review нужны настоящая reviewer identity/RBAC, lawful purpose и consent model, assignment/SLA/availability semantics, trusted-recipient transport, incident operations и отдельный controlled rollout.
...
На 15 июля 2026 года этапы 1-5 и инкременты Stage 6.1, 6.2 и 6.3a завершены. Полный Stage 6 остаётся в работе. Текущий код включает immutable action ledger, compensating rollback, event-driven recalculation assessments, historical resolver, deterministic Weekly Review, Hybrid Conversational Coach, normalized Recovery/Wearable Signals, controlled read-only Telegram daily/post-workout/weekly loops и default-off athlete-facing safety escalation boundary.
...
Следующая точка продолжения: Stage 6.3b, реальный staffed human review для red flags и спорных return-to-run сценариев. Он не начинается без reviewer identity/RBAC, lawful purpose/consent, assignment/SLA semantics, trusted-recipient boundary и incident operations. Telegram остаётся read-only каналом, а любые изменения плана проходят существующий preview/apply boundary.

## Порядок ближайших инкрементов

1. Добавить Stage 6.3b staffed review поверх готового athlete-facing escalation lifecycle: reviewer identity/RBAC, consent, assignment/SLA, trusted recipient и incident operations.
