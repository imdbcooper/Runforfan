...
**Статус: software implementation завершена; полный Stage 6 acceptance не закрыт. Инкременты 6.1, 6.2, 6.3a, default-off foundation 6.3b, operational controls 6.3c, evaluation foundation 6.4 и software release gate 6.5 реализованы. Staffed review enablement и product outcome criteria остаются незакрытыми operational/evidence gates.**
...
### Инкремент 6.5: deterministic replay и software release gate

- Production `compute_weekly_review` выполняется на versioned offline corpus с normal progression, pain/illness/profile/feedback/recovery overrides, partial/missing/unlinked/overdone fallbacks, wearable anomaly, same-day correction и bounded resume scenarios.
- Corpus покрывает полный allowlist `hold/deload/resume/conservative_progression`; input order perturbation обязана давать byte-stable output. Golden manifest fingerprint зафиксирован тестом.
- Executable gate сам сравнивает manifest с golden fingerprint и блокирует deploy при replay drift или открытом delivery/safety rollout flag. Gate запускается на новом immutable backend digest до переключения production services.
- Обязательный pre-image CI выполняет полный backend/PostgreSQL regression, включая 5% progression cap, prior-safe-baseline resume, current-safety recheck, immutable past/completed workouts, concurrent deload apply и compensating rollback, а также frontend tests/build.
- Software pass не равен operational enablement: staffed review остаётся `blocked_pending_human_checklist`, product outcomes остаются `insufficient_data_pending_retention_and_trust_measurement`.
- Replay version: `stage6-strategy-replay-v1`. Release gate version: `stage6-software-release-gate-v1`.
...
На 15 июля 2026 года этапы 1-5 завершены, а Stage 6 software implementation реализована и выпущена default-off. Полный Stage 6 acceptance остаётся открытым до operational staffed-review prerequisites и доказанных product outcomes. Текущий код включает immutable action ledger, compensating rollback, event-driven recalculation assessments, historical resolver, deterministic Weekly Review, Hybrid Conversational Coach, normalized Recovery/Wearable Signals, controlled read-only Telegram loops, athlete-facing safety escalation boundary, bounded reviewer workflow, operator-controlled audience, immutable aggregate evaluation runs и executable deterministic release replay.
...
Следующая точка продолжения не является ещё одним software foundation инкрементом: controlled staffed review можно рассматривать только после заполнения реального staffing/coverage/incident ownership checklist и kill-switch drill. Acceptance criterion по adherence/completion/retention/trust без роста pain/overload остаётся незакрытым до controlled evaluation с валидным измерением и достаточной выборкой. Текущие production delivery/safety flags остаются `false`.

## Порядок ближайших инкрементов

1. Не включать staffed review без реальных named reviewers, coverage, incident owner и drill evidence.
2. Не заявлять улучшение retention/trust до появления валидного инструмента и достаточной выборки.
3. Любое следующее изменение rule version повторно проходит replay и immutable evaluation gates.
