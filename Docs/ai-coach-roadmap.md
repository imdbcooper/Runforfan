...
На 13 июля 2026 года этапы 1 и 2 завершены. Текущий код зафиксирован после immutable action ledger, compensating rollback и persisted event-driven recalculation assessments. Production deployment не выполнялся.

Следующая точка продолжения: этап 3, historical resolver и первый read-only deterministic Weekly Review без LLM. Недельные стратегии `hold`, `deload`, `resume` и `conservative_progression` должны выпускаться следующим отдельным preview/apply инкрементом после read-only review.

## Порядок ближайших инкрементов

1. Реализовать historical resolver поверх immutable facts, events и plan versions.
2. Построить read-only deterministic Weekly Review с evidence и freshness.
3. Добавить offline replay и regression fixtures для Weekly Review.
4. Добавить confirmed weekly strategies `hold`, `deload`, `resume`, `conservative_progression`.
5. Подключить LLM только для объяснения и уточняющих вопросов после adversarial eval gate.
