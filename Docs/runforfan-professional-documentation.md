...
Основной UI: `http://127.0.0.1:5173/admin/`
...
- `Docs/assets/alpha-guide/*.png` - локальные скриншоты admin UI для HTML-гайда.
...
|---|---|---|
| Frontend | React, Vite, TypeScript, Tailwind v4, shadcn-style UI | Admin UI `/admin/`, формы, таблицы, графики, i18n, alpha workflows |
...
Admin UI открывается по адресу:
...
Vite настроен с `base: "/admin/"`, поэтому frontend должен обслуживаться под `/admin/`.
...
- frontend `/admin/` загружается без white screen;
