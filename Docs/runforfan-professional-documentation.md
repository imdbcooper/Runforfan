...
## 6. Экранная карта app UI

| Раздел | Назначение | Основные проверки |
|---|---|---|
| Сегодня | Ближайшая тренировка, текущая неделя, последние тренировки, полезные предупреждения | Active plan, readiness, current week, quick actions |
| План | Активная программа, ближайшая тренировка, недели и создание программы | Preview/create/activate, feedback, completion, recommendations |
| Добавить тренировку | Скриншоты, проверка данных, retry, CSV и история загрузок | Queue/retry, confirm/cancel, CSV duplicates, plan match |
| Прогресс | Пользовательская сводка, trends, best efforts, insights | Period filters, weighted metrics, advanced details |
| Профиль | Availability, physiology, constraints, safety, advanced zones | Save profile, recalc zones, manual HR override |
| Еще | Calendar, Workouts, Goals, Load, Zones, Performance, Settings | Specialist and technical workflows |

Frontend использует URL-based navigation с basename `/app`: основные routes — `/app/`, `/app/planning`, `/app/imports`, `/app/analytics`, `/app/profile`. Secondary routes: `/app/calendar`, `/app/activities`, `/app/goals`, `/app/load`, `/app/zones`, `/app/performance`, `/app/settings`.
...
| `POST` | `/imports/{batch_id}/retry` | Сбросить ошибку и повторно поставить screenshot batch в очередь |
...
Upload images
  ...
  -> enqueue batch as queued
  -> deterministic template recognition if known template
  -> vision LLM recognition if provider configured
  -> retry_scheduled for known temporary provider failures
  -> automatic worker retry up to configured max attempts
  -> candidate JSON
  -> validation checks
...
Пользователь может вручную повторить screenshot batch в состояниях `retry_scheduled`, `validation_failed`, `recognition_failed` и `rejected_no_llm_template`. `POST /imports/{batch_id}/retry` требует существующие screenshot sources и отсутствие уже созданной activity, сбрасывает attempts/last error и возвращает batch в `queued`. Неизвестные программные ошибки логируются и завершаются как `recognition_failed`, но не повторяются автоматически вслепую.
...
Production Caddy направляет `/app/*` во frontend nginx container. Frontend nginx должен отдавать `/app/sw.js`, `/app/manifest.webmanifest`, `/app/offline.html` и `/app/icons/*` как реальные static files. Missing PWA files не должны fallback-иться в `/app/index.html`, иначе браузер получит HTML вместо service worker/manifest.
...
- `Сегодня` показывает ближайшую тренировку и current week без ошибок console;
- Profile сохраняется и zones recalculated;
- `Тренировки` позволяет добавить запись вручную и пересчитывает derived metrics;
- `Добавить тренировку` создает candidate или показывает понятное состояние ошибки;
- retryable screenshot batch показывает `Повторить сейчас` и после retry возвращается в `queued`;
...
- Retryable screenshot errors можно повторить автоматически или вручную; CSV failures используют отдельный CSV flow.
