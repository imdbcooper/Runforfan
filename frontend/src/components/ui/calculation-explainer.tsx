...
export function CalculationExplainer({ title = "Why?", className, children }: CalculationExplainerProps) {
  return <details className={cn("group relative text-left", className)}>
    ...
    <div className="absolute right-0 z-20 mt-1 w-72 rounded-md border border-zinc-800 bg-zinc-950 p-3 text-[11px] leading-5 text-zinc-300 shadow-xl shadow-black/40">
      {children}
    </div>
  </details>
}
